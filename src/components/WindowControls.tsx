...
export function WindowControls({ closeOnly = false }: Props) {
  ...
  return (
    <div className="flex h-full shrink-0 items-center gap-0.5 pr-1">
      {!closeOnly && (
        <>
          <CtlButton ariaLabel={t("windowControls.minimize")} onClick={() => void w.minimize()}>
            <HugeiconsIcon icon={MinusSignIcon} size={12} strokeWidth={2} />
          </CtlButton>
          <CtlButton
            ariaLabel={maximized ? t("windowControls.restore") : t("windowControls.maximize")}
      ...
      <CtlButton ariaLabel={t("common.close")} onClick={() => void w.close()} danger>
