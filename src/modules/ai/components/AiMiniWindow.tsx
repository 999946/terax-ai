  ...
  return (
    <div className="flex shrink-0 items-center gap-2 border-b border-border/40 bg-muted/40 px-3 py-1.5">
      ...
      <span className="text-[11px] text-muted-foreground">
        {queueLen > 0 ? `· ${queueLen} ${t("ai.planQueued")}` : "· " + t("ai.planNoEditsQueued")}
      </span>
      ...
      <ContextContent className="w-64 text-[11px]">
        ...
        <ContextContentBody>
          <div className="flex items-center justify-between text-muted-foreground">
            <span>{t("ai.ctxModel")}</span>
            <span className="font-mono text-foreground">{modelLabel}</span>
          </div>
          <div className="mt-1 flex items-center justify-between text-muted-foreground">
            <span>{lastInput > 0 ? t("ai.ctxLastRequest") : t("ai.ctxEstimatedContext")}</span>
          ...
          {lastCached > 0 && (
            <div className="flex items-center justify-between text-muted-foreground">
              <span>{t("ai.ctxOfWhichCached")}</span>
              <span className="font-mono text-foreground">
          ...
          {reported > 0 && (
            <>
              <div className="mt-1.5 flex items-center justify-between text-muted-foreground">
                <span>{t("ai.ctxSessionInput")}</span>
                <span className="font-mono text-foreground">
                  {formatTokens(tokens.inputTokens)}
                </span>
              </div>
              <div className="flex items-center justify-between text-muted-foreground">
                <span>{t("ai.ctxSessionOutput")}</span>
                <span className="font-mono text-foreground">
              ...
              {tokens.cachedInputTokens > 0 && (
                <div className="flex items-center justify-between text-muted-foreground">
                  <span>{t("ai.ctxCacheHit")}</span>
                  <span className="font-mono text-foreground">{cacheRate}%</span>
                </div>
              )}
              {cost != null && (
                <div className="flex items-center justify-between text-muted-foreground">
                  <span>{t("ai.ctxSessionCost")}</span>
                  <span className="font-mono text-foreground">
        ...
        <ContextContentFooter>
          <span className="text-[10px] italic text-muted-foreground">
            {lastInput > 0
              ? t("ai.ctxLastRequestHint")
              : t("ai.ctxEstimatedHint")}
          </span>
        </ContextContentFooter>
      </ContextContent>
    </Context>
...
function EmptyState({ onPick }: { onPick: (text: string) => void }) {
  ...
  const suggestions = [
    {
      label: t("ai.suggestionExplainError"),
      hint: t("ai.suggestionReadTerminal"),
      icon: AlertCircleIcon,
      text: t("ai.suggestionExplainErrorText"),
    },
    {
      label: t("ai.suggestionGenerateCommand"),
      hint: t("ai.suggestionTellWhatToDo"),
      icon: TerminalIcon,
      text: t("ai.suggestionGenerateCommandText"),
    },
    {
      label: t("ai.suggestionSummarizeBuffer"),
      hint: t("ai.suggestionRecapActivity"),
      icon: FilterIcon,
      text: t("ai.suggestionSummarizeBufferText"),
    },
  ];
  return (
