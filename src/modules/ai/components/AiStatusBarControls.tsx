...
export function AiStatusBarControls() {
  ...
  return (
    <div className="flex items-center gap-0.5">
      ...
      <IconBtn
        title={`${miniOpen ? t("common.close") : t("ai.open")} ${t("ai.chatWindow")} (${fmtShortcut("⇧", MOD_KEY, "I")})`}
        onClick={toggleMini}
      >
    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button
          ...
          title={
            currentProviderHasKey
              ? `${t("ai.model")}: ${current.label}`
              : `${current.label} — ${t("ai.noKeyConfigured")}`
          }
        >
          {current.label}
          <HugeiconsIcon
        ...
        <div className="flex max-h-104 min-h-0">
          ...
          <div className="flex w-11 shrink-0 flex-col gap-0.5 overflow-y-auto border-r border-border/70 bg-muted/20 py-1.5">
            ...
            {customEndpoints.length > 0 && (
              <ProviderPill
                icon={PlugIcon}
                title={t("settings.models.openAiCompatible")}
...
function ProviderConfigureCTA({ providerId }: { providerId: ProviderId }) {
  ...
  return (
    <button
      type="button"
      onClick={() => useFloatingSettings.getState().openSettings("models")}
    ...
    >
      ...
      <span className="flex-1 truncate">
        {t("ai.configureProvider", { provider: p.label })}
      </span>
...
function CapabilityBars({ caps }: { caps: ModelCapabilities }) {
  ...
  return (
    <div className="ml-auto flex items-center gap-1.5">
      <CapBar icon={BrainIcon} value={caps.intelligence} label={t("ai.capIntelligence")} />
      <CapBar icon={FlashIcon} value={caps.speed} label={t("ai.capSpeed")} />
      <CapBar
        icon={CoinsDollarIcon}
        value={caps.cost}
        label={t("ai.capAffordability")}
      />
    </div>
  );
}
