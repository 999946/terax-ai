    ...
    return (
      <span key={`enable-${hint.preset.id}`} className={PILL_CLASS}>
        ...
        >
          ...
          <span>
            {t("lsp.enableLsp", { name: hint.preset.name })}
          </span>
        </button>
    ...
    <Popover>
      ...
      >
        <div className="mb-1 font-medium text-foreground">
          {t("lsp.stoppedTitle", { name: preset.name })}
        </div>
    ...
    <Popover>
      <span className={PILL_CLASS}>
        <PopoverTrigger asChild>
          <button type="button" className="flex items-center gap-1">
            <HugeiconsIcon icon={SourceCodeIcon} size={11} strokeWidth={2} />
            <span>{t("lsp.installLsp", { name: preset.name })}</span>
          </button>
        </PopoverTrigger>
      ...
      >
        <div className="mb-2 font-medium text-foreground">
          {t("lsp.languageServer", { name: preset.name })}
        </div>
        <p className="mb-2 text-muted-foreground">
          {t("lsp.notFoundOnPath", { command: preset.command })}
        </p>
        {install ? (
    ...
    <Popover>
      ...
      >
        ...
        <p className="mb-2 text-muted-foreground">
          ...
          {starting ? t("lsp.isStarting") : t("lsp.isRunning")}{" "}
          {t("lsp.forThisWorkspace")}
        </p>
        <div className="flex items-center gap-1.5">
          <button
            type="button"
